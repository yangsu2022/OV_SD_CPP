...
int32_t main(int32_t argc, char *argv[]) {
    ...
    options.add_options()
        ...
        ("p,precision", "Specify precision of SD model IR", cxxopts::value<std::string>()->default_value("FP16_static"))
    ...
    const std::string positive_prompt = result["text"].as<std::string>();
    ...
    if ( !std::filesystem::exists( model_path + "/" + precision )) {
        std::cout << "Model path: " << model_path + "/" + precision << "not exist\n";
        exit(0);
    }
