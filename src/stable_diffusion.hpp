...
std::vector<float> LMSDiscreteScheduler(
    int32_t num_train_timesteps = 1000, float beta_start = 0.00085f,
    float beta_end = 0.012f, std::string beta_schedule = "scaled_linear",
    ...
    std::vector<float> trained_betas = std::vector<float>{}) {
        ...
        if (!trained_betas.empty()) {
            ...
            betas.push_back(beta_start +
                            (beta_end - beta_start) * i / (num_train_timesteps - 1));
            }
            ...
            std::cout << " beta_schedule must be one of 'linear' or 'scaled_linear' "
                    << std::endl;
        }
        for (float b : betas) {
        ...
        for (int32_t i = 1; i <= (int)alphas.size(); i++) {
            ...
            float alphas_cumprod =
                std::accumulate(std::begin(alphas), std::begin(alphas) + i, 1.0,
...
std::vector<float> std_randn_function( uint32_t seed )
{
    std::vector<float> noise;
    {
        std::mt19937 gen{ static_cast<unsigned long>( seed ) };
        std::normal_distribution<float> normal{0.0f, 1.0f};
        noise.resize( 64 * 64 * 4 * 1);
...
std::vector<uint8_t> vae_decoder_function( ov::CompiledModel& decoder_compiled_model, std::vector<float>& sample )
{
    ...
    std::for_each(sample.begin(), sample.end(), [coeffs_const](float& i) { i *= coeffs_const; });
    ov::Shape sample_shape = {1,4,64,64};
    ...
    float mul_const{0.5};
    std::transform(decoder_output_vec.begin(), decoder_output_vec.end(), decoder_output_vec.begin(), [&mul_const](auto &c) { return c * mul_const; });
    float add_const{0.5};
    std::transform(decoder_output_vec.begin(), decoder_output_vec.end(), decoder_output_vec.begin(), [&add_const](auto &c) { return c + add_const; });
    std::transform(decoder_output_vec.begin(), decoder_output_vec.end(), decoder_output_vec.begin(),
                   [=] (auto i) { return std::clamp(i, 0.0f, 1.0f); });
...
std::vector<float> unet_infer_function( ov::CompiledModel& unet_model, std::vector<int64_t>& vector_t, std::vector<float>& latent_input_1d, std::vector<float>& text_embedding_1d )
{
    ...
    for (auto input: unet_model.inputs()) {
        ...
        if (name == "sample") { // latent_model_input
            ov::Shape latent_shape = {2,4,64,64};
    ...
    std::vector<float> noise_pred_uncond_vec(noise_pred_ptr, noise_pred_ptr + ( 64 * 64 * 4  ));
    std::vector<float> noise_pred_text_vec(noise_pred_ptr + ( 64 * 64 * 4 ), noise_pred_ptr + ( 64 * 64 * 4 * 2));
...
std::vector<float> diffusion_function( ov::CompiledModel& unet_compiled_model, uint32_t seed, int32_t step, std::vector<float>& latent_vector_1d,std::vector<float>& text_embeddings_2_77_768 )
{ 
    ...
    std::transform(latent_vector_1d.begin(), latent_vector_1d.end(), latent_vector_1d_new.begin(), [&n](auto &c) { return c * n; });
    ...
    for ( int32_t i = 0; i < static_cast<int>(sigma.size()) - 1; i++ )
    {
        ...
        std::vector<float> noise_pred_1d = unet_infer_function( unet_compiled_model, t, latent_model_input, text_embeddings_2_77_768 );
        ...
        logger.log_value(LogLevel::DEBUG, "DEBUG-noise_pred_1d.size() after unet_infer_function: " , noise_pred_1d.size());
        ...
        for (int32_t curr_order = 0; curr_order < order; curr_order++)
        {
            ...
            auto integrated_coeff = boost::math::quadrature::trapezoidal(f, static_cast<double>(sigma[i]), static_cast<double>(sigma[i+1]), 1e-4); 
        ...
        for ( int32_t m = 0; m < order; m++ ) {
            ...
            std::for_each(rev_derivative[m].begin(), rev_derivative[m].end(), [coeffs_const](float& i) { i *= coeffs_const; });
        }
        // sum of derivative
        std::vector<float> derivative_sum = rev_derivative[0];
        if (order > 1) {
            for (int32_t d = 0; d < order - 1; d++) {
                std::transform(derivative_sum.begin(), derivative_sum.end(), rev_derivative[d+1].begin(), derivative_sum.begin(), [](float x, float y) { return x + y; });
            }
        }
        // latent + sum of derivative
        std::transform(derivative_sum.begin(),derivative_sum.end(), latent_vector_1d_new.begin(), latent_vector_1d_new.begin(), [](float x, float y) { return x + y; }); 
...
std::vector<std::vector<int32_t>> tokenizer_infer_function( ov::CompiledModel& tokenizer_model, std::string prompt ){
    ...
    logger.log_value(LogLevel::DEBUG, "tokenizer_encoder_input_port.get_partial_shape(): ", tokenizer_encoder_input_port.get_partial_shape());
...

{
    ...
    ov::Tensor text_embeddings_input_tensor(clip_input_port.get_element_type(), clip_input_shape, current_tokens.data());
...
std::vector<int32_t> pre_process_function(std::unordered_map<std::string, int>& tokenizer_token2idx, std::string prompt){
...
std::vector<float> prompt_function( ov::CompiledModel& text_encoder_compiled_model,std::string const& prompt_positive_str, std::string const& prompt_negative_str )
{
    // read vocab 
...
std::vector<ov::CompiledModel> SD_init(std::string& model_path, std::string& device, std::string& precision, std::map<std::string,float>& lora_models){
    ...
    if (!lora_models.empty()){
        ...
        ov::CompiledModel text_encoder_compiled_model = core.compile_model(text_encoder_model, device);
        ov::CompiledModel unet_compiled_model = core.compile_model(unet_model, device);
        SD_compiled_models.push_back(text_encoder_compiled_model);
        SD_compiled_models.push_back(unet_compiled_model);
    }
...
void stable_diffusion( std::string positive_prompt = std::string{}, std::string output_png_path = std::string{}, int32_t step = 20, uint32_t seed = 42, std::string negative_prompt = std::string{}, 
    ...
    if (read_np_latent) {
        ...
        latent_vector_1d = std_randn_function(seed);
    }
    logger.log_vector(LogLevel::DEBUG, "randn output: ", latent_vector_1d, 0, 20);

    auto sample = diffusion_function( SD_models[1], seed, step, latent_vector_1d, text_embeddings );
    ...
    auto output_decoder = vae_decoder_function( SD_models[2], sample );
