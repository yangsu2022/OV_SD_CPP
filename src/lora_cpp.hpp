...
safetensors_Str 
stringToSafetensorsStr(const std::string& str) {
...
void 
releaseSafetensorsStr(safetensors_Str& str) {
    delete[] str.ptr;
}
...
void print_origin_layer(const std::string& key) {
	...
	for (int i = 0; i < f.num_tensors; i++) {
		...
		std::cout << "]\n\toffsets: [" << static_cast<long long>(t.begin_offset_bytes) << ", " << static_cast<long long>(t.end_offset_bytes) << "]\n\tpointer: " << t.ptr << "\n\n";
	}
}
...
modify_layer(const std::string& key, float alpha = 0.75f) {
    ...
    for(int i = 0; i < f.num_tensors; i++) {
        ...
        if (key.find("text") != std::string::npos) {
            // cut out "lora_te_"
            std::string layer_infos = key.substr(key.find(LORA_PREFIX_TEXT_ENCODER) + LORA_PREFIX_TEXT_ENCODER.length() + 1);
        ...
        // matmul with fp16
        Eigen::Map<Eigen::Matrix<ov::float16, Eigen::Dynamic, Eigen::Dynamic>> mat2d_f16_0(raw_data_0.data(), shape_vec_0[0], shape_vec_0[1]);
        Eigen::Map<Eigen::Matrix<ov::float16, Eigen::Dynamic, Eigen::Dynamic>> mat2d_f16_1(raw_data_1.data(), shape_vec_1[0], shape_vec_1[1]);
        ...
        // RowMajor
        Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> mat2d_f_0(float_data_0.data(), rows_0, cols_0);
        Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> mat2d_f_1(float_data_1.data(), rows_1, cols_1);
    ...
    for (int i = 0; i < lora_name_list.size(); i++) {
        ...
        // std::cout << "Name: " << name << ", Type: " << type << ", Shape (" << weight_mat.rows() << ", " << weight_mat.cols() << ")" << std::endl;
